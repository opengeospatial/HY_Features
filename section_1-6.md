...
It is important to note that the catchment network model follows only the hydrological significance governed by physical laws. Defining the catchment as the principal link between inflow and outflow nodes located on the bounding divide, the catchment topology conforms in general to the arrangment of links and nodes, as defined in ISO 19107: Spatial Schema, even if the geometric realisation of the topological link and node is not designed. Given that catchment topology is hydrologically determined independent from geometric realisation, any type of catchment representation may be applied to visualise the catchment topology using common geometric shapes. Even if an context-related geometric realisation of the connecting link may be requested, an application-specific re-definition of the topological network is not necessary.    
...
**[\*\*\* I intend to remove section 6.6. on Surface Hydro Feature, since the conceptual separation of watercourses is already described in 6.5, as well as the surface water confines --- furthermore, when atmospheric and subsurface are moved to annexes (see line 421), it doesn't make sense to retain the a surface hydro feature section  repeating 6.5 \*\*\*]**(brief outline with reference to the normative section 7, in favour of completeness of the hydro cycle, against the background of the general holistic catchment approach) 
