...
### 6.6 Hydrographic network, channel network and station network - different views of catchment topology
It is the intended goal of this standard to provide the conceptual basis for an identity-based linkage of data and data products representing the same catchment by reference to the common unit of study. The common acceptance of the holistic catchment approach in hydrology allows to consider different application-specific appearances of a catchment as variations of a basic, hydrologically significant pattern. Interpreting the hydrographic network (of streams), or the channel network (of watercourses), as one of many appearances the same catchment may be recognized across hydrologic disciplines, the typical relationships describing the connectivity of a catchment in a set of catchments can be transferred onto any contextual catchment view, regardless of its geometric representation in a particular data set or product. Following this basic conceptual scheme, the hydrosphere is understood as a network of catchments connected at inflow and outflow nodes, or along the edges of the (watershed) divide. This standard defines a hydrologically determined topology model of *outfalls*, acting as inflow or outflow nodes, and *catchmments* acting as link between them. This catchment network pattern can be applied to application-specific network concepts such as hydrographic network or channel network, as well as to their various representations as (poly)lines and polygons. For example, a fixed landmark on a water body, a section separating a watercourse, or a station placed in the network, may be considered as outfall (outflow node) of a contributing catchment (link); a representing flowpath may be drawn from (inflow) node to (ouflow) node linked through the represented catchment between both. As for the hydrographic network, the catchment topology model can be applied to any logical or virtual network, e.g. a network of logically connected hydrometric stations.

It is important to note that the catchment network model follows only the hydrological significance governed by physical laws. Defining the catchment as the principal link between inflow and outflow nodes located on the bounding divide, the catchment topology conforms in general to the arrangment of links and nodes, as defined in ISO 19107: Spatial Schema, even if the geometric realisation of the topological link and node is not designed in the hydrologic feature model. Given that topology is hydrologically determined independent from geometric realisation, common geometric shapes can be used to visualise the catchment topology in several network views, whereby the geoemtric features may be connected at the representation level, or not. Even if a specific context-related geometric realisation of the connecting catchment link may be requested, a representation-specific re-definition of the topological connectivity is not necessary. Application-specific realisations of the catchment topology may use the general catchment representation type, or one of the specialisation typical for the hydrology domain defined in this standard.  

**[end of new section 6.6, re 'add a statement about the intent of the model and the relationship to ISO topology in the text']**
