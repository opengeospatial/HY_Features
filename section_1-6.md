...
Recognized across hydrologic disciplines as the basic unit of study where hydrologic processes (such as flow accumulation) take place, the catchment may be represented in various ways in different sub-hydrology-domain information models. Catchment boundary, catchment area, and linear flowpath, are the most common geometric representations of a catchment, while a network structure in its entirety represents a catchment as a set of related features. Most common in use is the geospatial repesentation of such networks displaying the geoemtric representations of the topologically connected features. Physical, topographic, or other thematic maps, but also geoschematic views, showing the hydrographic network of water bodies, the network of channels, or a hydrometric network of stations are examples of cartographic views that in its entirety, separately or as set of layers, represents a catchment without describing the displayed network parts in detail. Figure 4 shows different types of geospatial data overlaying the represented Missouri region.
...
a) Catchment boundary, b) Catchment area, c) Hydrologic unit containing sub-units, d) Abstract lowline and common outlet, e) Network of flow lines at different level of detail, f) Network of logically connected monitoring stations  
...
The water body is by the definition given in the UNESCO-WMO "International Glossary of Hydrology" [3] understood as the mass of liquid water, accumulated on or below the land surface as a flowing body of water (stream), which in some parts may have water that is not moving or flowing. Although referencing in general this definition, this standard is concerned with water bodies that form the hydrographic network on the land surface, including lakes and reservoirs. A conceptual model capturing the specifics of bodies of groundwater will be provided with the GroundwaterML 2.0 under development [\*\*\*see annex ..., insert reference to GWML2\*\*\*]. 

Water bodies, flowing or stagnant, are aggregated in the hydrographic network, and may be further divided into parts confined by cross- or longitudinal stream sections. Each water body part may be stratified into distinct horizontal layers of consistent thermal or salinity characteristics, or oxygen or nutrient content, but also into virtual storage zones of a managed reservoir. Being a mass of liquid water, the occupied landform determines the shape of the contained water body; having a surface exposed to the atmospshere, the extent is bound to atmospheric pressure exerted on surface. 

Considering that the channel network (drainage pattern) generally exists independently of whether water flows therein or not, each channel is understood as a containing unit continuously or periodically carrying water. Referencing to drainage pattern generally occuring on the land surface, this standard is concerned with channels that form the system to potentially connect water bodies on the land surface, but does not cause their interaction with other water bodies part of the hydrogrgaphic network. Each channel may be further divided into channel parts between vertical bed profiles which may contain, or not contain, flowing or stagnant parts of a water body. For example, normally stagnant waters may or may not be connected to streams during flood events, or streams may be interrupted to pools of stagnant water in periods of drought or not. A conceptual model capturing the specifics of aquifers containing groundwater will be provided with the GroundwaterML 2.0 under development [\*\*\*see annex ..., insert reference to GWML2\*\*\*]. 

******

Following the separation of concerns described above, the network of permanent and temporary watercourses may be described by both the hydrographic network of water bodies as well as the containing network of channels.  


c) water body part / channel part carrier of the reference location used to place an arbitrary location along a linear representation of a catchment 

Watercourses have names within common experience, but may have different names in different contexts.
