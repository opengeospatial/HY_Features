...
The HY\_RiverReferenceSystem feature class specializes the ISO LinearCS feature type for a linear coordinate system using inflow and outflow nodes on the linear flowpath. The origin of the river reference system is set at the **outfall to be placed** ~~an already located outfall~~. The geometric shape is defined by the linear flowpath realizing the catchment between the origin and another already located outfall upstream or downstream of the origin. **The position on the flowpath is provided as a distance from the origin to an already located outfall (upstream or downstream), or as percentage of the distance between the two bounding outfalls upstream and downstream.** HY\_RiverReferenceSystem inherits the *axis* property, and carries the associations *linearElement* and *locatedStart*.
