...
AHGFGhostNode
 ...
 |In the AHGF, hydrometric features are the combination of AHGFGhostNode features and those AHGFNode ghost node features that realize contracted nodes representing hydrometric stations. Thus, from a AHGF perpective, a hydrometric network can be seen as the collection of logically connected hydrometric ghost node features (from both classes), which provides a specific realization of a contracted catchment, which monitors that catchment.
...
|Measures are not yet explicitly
