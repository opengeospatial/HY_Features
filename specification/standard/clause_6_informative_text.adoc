...
hydro nexus may take on the topological role of a node but not
represented geometrically as a single point.
...
NHDPlus [10]. In this case, the catchment network forms a continuous
...
Hydrological Geospatial Fabric [11] to describe catchments, and
...
[#figure8]
.Catchment Catchment hierarchy with smaller catchments (C1, C2, C3) part of a dendritic catchment network, which is also a catchment and corresponding HY_Features UML classes.
...
[#figure9]
.Catchment area (grey area A) and flowpath realization (red line F) that connects catchment inflow to outflow for a defined a hydrologically significant unit (C1) and corresponding HY_Features UML classes.
...
where hydro nexus nodes n2 and n3 have one and two contributing
catchments respectively, and both have two receiving catchments. Such
...
collapsing their outflow nexuses n2 and n3 into a single virtual inflow
hydro nexus, accumulates all the flow from catchments D and F in the
...
topologically realize catchments as a solid bounded by inflow/outflow
...
connect hydrologic phenomena and observations across a range of the
scales.
...
which water may or may not flow [9]. A network of channels (or drainage
...
channels can also be thought of as realization of a hydro nexus that
drains the sub-catchment that feeds that waterbody. The catchment
...
hydro nexus, may be the reference location for stream gages downstream
and the next downstream bridge.
...
flowpath linear element and hydro nexus referent. It's worth mentioning
...
flowpaths and inflow and outflow hydro nexus locations (e.g. the
...
bounds the flowpath. The new network location is can be expressed as a
...
location, such as a stream gage, and is to be a hydro nexus that is
...
flowpath between the location (e.g. stream gage) and the inflow of the
...
of the downstream catchment, it's location is known to be the upstream
or downstream end of one of the associated catchments' flowpaths.
...
location to be placed cannot become a hydro nexus with associated
...
[#figure17]
.Position (yellow dot) of an outflow node referencing an inflow node, expressed as a measured distance from a upstream referent (red dot) and representative UML classes.
...
to be an inflow or outflow of a catchment.
...
[#table1]
.Table 1: HY_Features river positioning concepts comparable with the OGC Linear referencing model
...
another feature along an axis, +

has _distance expression_ and _distance description_,
...
[#table2]
.Table 2: HY_Features concepts comparable with the OGC Network navigation model
[width="100%",cols="34%,33%,33%",]
|=======================================================================
a|
*HY_Features concept*
